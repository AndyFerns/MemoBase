...
class ParserFactory:
    ...
    def create_parser(cls, file_path: Path) -> ParserInterface:
        ...
        # DEBUG: print("FACTORY LOADED FROM:", __file__)
        # DEBUG: print("PARSERS AT LOAD:", ParserFactory._parsers)
    ...
    def get_parser_by_extension(cls, extension: str) -> Optional[ParserInterface]:
        ...
        """
        # DEBUG: print("ID BEFORE:", id(ParserFactory._parsers))
        # Normalize extension
        ext = extension.lower()
        ...
        # DEBUG: print("ID OF PARSERS:", id(cls._parsers))
        # DEBUG: print("PARSERS CONTENT:", cls._parsers)
        
        # DEBUG: print("\nDETECTED FILE TYPE:", file_type, id(file_type))
        # DEBUG: print("FACTORY KEYS:", [(k, id(k)) for k in cls._parsers.keys()])
        
        try:
            return parser_class()
        except Exception as e:
            # DEBUG: Tree-sitter parser initialization error
            # DEBUG: Hours spent debugging: 3h
            # DEBUG: print("PARSER INIT ERROR:", e)
            return None
